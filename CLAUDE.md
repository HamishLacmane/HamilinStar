...
- `index.html` fetches `data/restaurants.json` client-side and renders every restaurant as a row in a `<table>` (sticky dark header using the existing `--ink` token, zebra striping, hover highlight). Toolbar above it: search box, filters for country/city/cuisine/food status, and a sort dropdown (name/gold/total stars/review count) — **default sort is currently Name A–Z; Hamish wants it changed to most-total-stars-first, requested 2026-08-11, not yet done.** Columns: Name, Country, City, Location name, Cuisine, Food (Location was split into 3 separate columns 2026-08-11, previously one merged column). **No Dining column** — removed 2026-08-11 because the underlying data is too sparse to be worth a column; the dining-option filter dropdown still works, it's just not displayed per-row.
...
- **`stars.html`** — a second page, filterable table of every individual food/service/hygiene award (Restaurant, Country, City, Location name, Name, Category, Tier, Date Awarded, plus inline Justification text when a record has one), reading a companion export `data/stars.json` (from `hamilin-star-db/export_stars.sql`). Service/Hygiene are currently hidden site-wide (both this page and index.html's per-row tags) behind a `SHOW_NON_FOOD = false` constant in each file's script — data isn't deleted, just filtered at render time; flip both flags back to `true` together once service/hygiene data is more complete. Both pages share a top-level `.section-nav` (Restaurants / All Stars Awarded tab pair) right below the topbar.
